...
// This remains the standard "simplest" way for this task in Typst
show text: it => {
  let target = "E.R. Muise"
  if it.text.contains(target) {
    let parts = it.text.split(target)
    let new_content = ()
    
    for i in range(parts.len()) { 
      if parts.at(i).len() > 0 {
        new_content.push(text(parts.at(i)))
      }
      if i < parts.len() - 1 { 
        // Add the BOLDED target between parts
        new_content.push(strong("Muise, E.R.")) 
      }
    }
    new_content.sum() 
  } else {
  ...
  body // The show rule defined above will apply to this body content
}
